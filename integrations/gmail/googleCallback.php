...
if ($err || !$profileResp) {
    header('Location: /HOTEL-MANAGEMENT-SYSTEM/frontend/login.php?error=google_profile_error');
    exit;
}
...
if (!$email) {
    header('Location: /HOTEL-MANAGEMENT-SYSTEM/frontend/login.php?error=google_no_email');
    exit;
}
            ...
            if ($insert2->execute()) {
                ...
                if ($conn->errno === 1062) {
                    ...
                    if ($insert2->execute()) {
                        ...
                        header('Location: /HOTEL-MANAGEMENT-SYSTEM/frontend/login.php?error=google_db_error');
                        exit;
                    }
                } else {
                    header('Location: /HOTEL-MANAGEMENT-SYSTEM/frontend/login.php?error=google_db_error');
...
header('Location: /HOTEL-MANAGEMENT-SYSTEM/index.php?google_success=1');
exit;
